...
function match(page, url, typeMask, docDomain, sitekey) {
    ...
    //akshay hundia
    //chrome.extension.getBackgroundPage().console.log("filtering:"+url);
    ...
    exports.checkWhitelisted = (page, frame, typeMask) => {
        ...
        if (frame) {
            ...
            while (frame && !filter) {
                ...
                //  chrome.extension.getBackgroundPage().console.log(filter);
                filter = match(page, frame.url, typeMask, docDomain, sitekey);

                //chrome.extension.getBackgroundPage().console.log("docDomain:"+filter);
                // chrome.extension.getBackgroundPage().console.log(sitekey);
                //chrome.extension.getBackgroundPage().console.log(parent);
                let hostweb = frame.url.host.replace(/^www\./, "");
                // chrome.extension.getBackgroundPage().console.log(hostweb); 
                chrome.storage.sync.get('disabled', function(result) {
                    if (result && result != undefined) {
                        //  chrome.extension.getBackgroundPage().console.log(result.disabled);
                        //    chrome.extension.getBackgroundPage().console.log(result.websites[hostweb]);
                        if (result.disabled && hostweb != undefined && result.disabled[hostweb] != undefined) {
                            var mins = new Date().getTime() / 60000;
                            var oldmins = parseInt(result.disabled[hostweb]);
                            // chrome.extension.getBackgroundPage().console.log(hostweb+":"+(mins-oldmins));
                            if (mins - oldmins > 30) {
                                if (filter) {
                                ...
                                chrome.storage.sync.get("disabled", function(obj) {
                                    ...
                                    // chrome.extension.getBackgroundPage().console.log ( "Updated " + hostweb + " to " + mins );
                                    //});
                                });
                            }
                        }
                    }
                    //chrome.extension.getBackgroundPage().console.log("result:"+result.seconds);
                });
                //Akshay Code Done
                frame = parent;
            }
            //  chrome.extension.getBackgroundPage().console.log("--"+filter.disabled);
...
port.on("filters.isPageWhitelisted", (message, sender) => {
    chrome.extension.getBackgroundPage().console.log("Akshay: isPageWhitelisted");
...
function revalidateWhitelistingState(page) {
    ...
    //   chrome.extension.getBackgroundPage().console.log(backgroundPage);
    //});
    //var markup = document.documentElement.innerHTML;
    // chrome.extension.getBackgroundPage().console.log(page);
    //chrome.extension.getBackgroundPage().console.log(markup);
